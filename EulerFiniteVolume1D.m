classdef EulerFiniteVolume1D < handle & utilitiesClass
    ...
    methods
        ...
        function realtimePlot(obj)
            set(obj.sim.plot, 'XData', obj.mesh.x, 'YData', obj.euler.state.rho);
            drawnow;            
        end
        ...
        % Compute One Explicit Euler Time Step
        function explicitRK2TimeStep(obj)
            
            obj.getResidual();
            Qtemp               = obj.euler.state.Q;
            
            K1                  = obj.sim.dtt.*(obj.sim.Residual./obj.mesh.dx);
            obj.euler.state.Q   = obj.euler.state.Q + 0.5*K1;
            obj.updateBoundaryConditions();
            obj.getResidual();
            
            obj.euler.state.Q   = Qtemp + obj.sim.dtt.*(obj.sim.Residual./obj.mesh.dx);   
        ...
        function explicitRK4TimeStep(obj)
            ...
            K1                  = obj.sim.dtt.*(obj.sim.Residual./obj.mesh.dx);
            obj.euler.state.Q   = Qtemp + 0.5*K1;
            obj.updateBoundaryConditions();
            obj.getResidual();
            
            K2                  = obj.sim.dtt.*(obj.sim.Residual./obj.mesh.dx);
            obj.euler.state.Q   = Qtemp + 0.5*K2;
            obj.updateBoundaryConditions();
            obj.getResidual();
            
            K3                  = obj.sim.dtt.*(obj.sim.Residual./obj.mesh.dx);
            obj.euler.state.Q   = Qtemp + 1.0*K3;
            obj.updateBoundaryConditions();
            obj.getResidual();
            
            K4                  = obj.sim.dtt.*(obj.sim.Residual./obj.mesh.dx);
            
            obj.euler.state.Q   = Qtemp + (K1 + 2*K2 + 2*K3 + K4)/6;   
        ...
        % Perform Explicit Euler First Order Time Marching
        function solveExplicitRK2TimeAccurate(obj)    
            
            obj.sim.t   = 0;    
            
            while obj.sim.t     < obj.sim.tfinal
                
                obj.explicitRK2TimeStep(); 
                obj.sim.t       = obj.sim.t + obj.sim.dtt;    
        ...
        function solveExplicit(obj)
            ...
            if obj.sim.realplot
                obj.sim.plot = plot(NaN, NaN, 'Color', 'black', 'LineWidth', 1);
            end
